...
* Introduction
Ceci est ma configuration personnelle d'Emacs, inspiré par la configuration
...
** PDFs-tools
| Touche | Action                               | Détails / Couleur              |
|--------+--------------------------------------+--------------------------------|
| C-s    | Recherche avant                      | isearch-forward                |
| C-r    | Recherche arrière                    | isearch-backward               |
| n      | Aller à la correspondance suivante   | pendant isearch                |
| p      | Aller à la correspondance précédente | pendant isearch                |
| +      | Zoom avant                           | facteur pdf-view-resize-factor |
| -      | Zoom arrière                         | facteur pdf-view-resize-factor |
| R      | Rotation du PDF                      | 90° (sens horaire)             |
| h      | Surbrillance                         | couleur par défaut (jaune)     |
...
| g     | Première page                        | Retour au début du PDF         |
| G      | Dernière page                        | Aller à la fin du PDF          |
| e      | Aller à une page spécifique          | Demande le numéro de page      |
| r      | Recharger le PDF                     | Rafraîchit le document         |

+ clique droit pour changer de couleur ou supprimer un soulignement
+ C-s et C-r peuvent être tapés plusieurs fois pour sauter d’une occurrence à l’autre
...
Le paquet drag-stuff permet de déplacer des lignes ou des blocs
de texte vers le haut ou le bas avec des raccourcis clavier.

| Mode normal et visual | Action                                    |
|-----------------------+-------------------------------------------|
| M-j                   | déplacer la ligne ou le bloc vers le haut |
| M-k                   | déplacer la ligne ou le bloc vers le bas  |
|-----------------------+-------------------------------------------|
...
** Latex
| Raccourci | Fonction                         | Description courte                                |
|-----------+----------------------------------+---------------------------------------------------|
| C-c C-c   | Compiler                         | Compile (LaTeX, BibTeX, View, etc.)               |
| C-c C-v   | Voir PDF                         | Ouvre le document compilé                         |
| C-c C-e   | Environnement                    | Insère \begin{...} \end{...}                      |
| C-c C-m   | Commande (macro)                 | Insère une commande LaTeX                         |
| C-c C-s   | Section                          | Insère section / subsection / chapter             |
| M-RET     | Nouvel item                      | Ajoute \item dans liste                           |
| C-c C-n   | Section suivante                 | Navigation vers section suivante                  |
| C-c C-p   | Section précédente               | Navigation vers section précédente                |
| C-c C-l   | Erreur suivante                  | Va à l’erreur après compilation                   |
| TAB       | Complétion                       | Complète commandes, labels, citations             |
| C-c ;     | Commenter / Décommenter          | Commente ou décommente la région                  |
| C-c b     | my/vertico-bibtex                | Ouvre la complétion BibTeX pour insérer une clé   |
| C-c p     | bibtex-completion-open-pdf       | Ouvre le PDF associé à la clé BibTeX (si présent) |
| C-c `     | Aller à la première erreur LaTeX |                                                   |
*** Exemples
**** Dans refs.bib
#+BEGIN_EXAMPLE
@article{Knuth1984,
...
#+END_EXAMPLE
**** Dans pdf
Le nom doit correspondre à la clé BibTeX.
+ Knuth1984.pdf
**** Ouvrir le PDF avec la clé
+ Place le curseur sur la clé et fait M-x bibtex-completion-open-pdf
...
L'apprentissage profond est très utilisé en intelligence artificielle
\cite{Smith2020}.

Pour apprendre LaTeX, on peut lire un livre spécialisé
\cite{Dupont2018}.
...
**** Dans emacs
+ C-c C-c + Latex pour compiler
+ C-c C-c + view pour visualiser le pdf 
#+BEGIN_SRC emacs-lisp
...
*** Couleur du fichier après enregistrement
#+BEGIN_SRC emacs-lisp
...
#+RESULTS:
| abbrev-mode | org-tempo-setup | toc-org-enable | org-superstar-mode | #[0 \301\211\207 [imenu-create-index-function org-imenu-get-tree] 2] | turn-on-org-cdlatex | rainbow-mode | org-indent-mode | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-fold-show-all append local] 5] | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-babel-show-result-all append local] 5] | org-babel-result-hide-spec | org-babel-hide-all-hashes |
...
#   (nyan-animate-nyancat t)    ;; Chat animé
#   :config
#   (nyan-mode 1)
...
*** Company headers (autocomplétion pour C/C++)
#+begin_src emacs-lisp
;; ----------------------------
...
#+RESULTS:
| (lambda nil (setq indent-tabs-mode nil) (setq tab-width 4)) | doom-modeline-env-setup-python |
...
;; C-c n f : chercher ou créer une note
;; C-c n g : ouvrir le graphe visuel dans le navigateur
;; C-c n i : insérer un lien vers une autre note
;; C-c n c : capturer une nouvelle note
;; C-c n t : ajouter un tag à une note
;; C-c n a : ajouter un alias à une note
;; C-c n j : créer ou ouvrir la note du jour (Daily)
;; C-c n e : rechercher un fichier Org-roam via consult
;; C-c n b : afficher les backlinks
;; C-c n B : backlinks récursifs
...
1️⃣ Créer ta première note
- Taper C-c n f
- Saisir le titre : "Projet Alpha"
- Emacs crée le fichier :
  ~/.emacs.d/Notes/roam/projet-alpha.org
- Contenu initial :
  #+title: Projet Alpha
  #+date: 2025-10-29

2️⃣ Ajouter du contenu
- Dans projet-alpha.org :
  Nous devons créer une application web pour gérer les tâches.

  Idées :
  - Utiliser Emacs pour les notes.
  - Prévoir un système de backlog.

  Voir aussi :
  - [[Projet Beta]]

3️⃣ Créer une note liée (backlink)
- Taper C-c n f
- Nouvelle note : "Projet Beta"
- Contenu :
  #+title: Projet Beta
  Cette note est liée à Projet Alpha.

4️⃣ Insérer un lien dans une autre note
...
|-----------------------------|----------------|-----------------------------------------------|
| Créer ou chercher une note  | C-c n f        | Fichier .org créé ou ouvert                   |
| Insérer un lien             | C-c n i        | Lien interne créé                             |
| Ajouter un tag              | C-c n t        | Tag ajouté à la note                           |
| Ajouter un alias            | C-c n a        | Alias pour retrouver la note                  |
| Notes journalières          | C-c n j        | Note du jour créée                             |
...
* mu4e
** Cheat sheet mu4e
| Action                        | Touche / Commande | Exemple concret                | Résultat                     |
|-------------------------------+-------------------+--------------------------------+------------------------------|
| Lancer mu4e                   | M-x mu4e          | M-x mu4e                       | Ouvre l’interface mail       |
| Aller dans l’INBOX            | Ji                | Ji → /Gmail/INBOX              | Affiche les mails reçus      |
...
| Aller au mail précédent       | p                 | p                              | Message précédent            |
| Répondre                      | r                 | r                              | Répond à l’expéditeur        |
| Répondre à tous               | R                 | R                              | Répond à tous                |
| Nouveau mail                  | C                 | C                              | Ouvre le buffer de rédaction |
| Envoyer un mail               | C-c C-c           | Écrire → C-c C-c               | Envoi via SMTP               |
| Marquer comme lu              | !                 | !                              | Marque le mail comme lu      |
| Marquer non lu                | ?                 | ?                              | Marque non lu                |
| Supprimer (Gmail Trash)       | d                 | d                              | Déplace vers Corbeille       |
| Archiver                      | a                 | a                              | Archive le mail              |
| Ajouter un drapeau            | f                 | f                              | Ajoute un flag               |
| Télécharger pièce jointe      | A s               | A s                            | Sauve la pièce jointe        |
| Voir pièces jointes           | A                 | A                              | Liste des PJ                 |
| Changer de dossier            | J                 | J → sélectionner un dossier    | Navigation rapide            |
| Mettre à jour les mails       | u                 | u                              | Sync + index                 |
...
| M-SPC  | Insert / Emacs  | Active le leader sans insérer d’espace |
|--------+-----------------+----------------------------------------|
** Commandes
...
| *A.I.*                        |                                                         |
| a                           | Menu A.I.                                               |
| a a                         | Demander à Ellama sur la sélection                      |
| a e                         | Menu Ellama – améliorer                                 |
| a e g                       | Améliorer la grammaire avec Ellama                      |
| a e w                       | Améliorer la formulation avec Ellama                    |
| a i                         | Chat avec Ellama                                        |
| a p                         | Sélectionner le fournisseur Ellama                      |
| a s                         | Résumer la sélection avec Ellama                        |
| a t                         | Traduire la sélection avec Ellama                       |
|                             |                                                         |
| *Bookmarks / Buffers*         |                                                         |
| b                           | Menu Bookmarks & Buffers                                |
| b b                         | Aller à un buffer                                       |
| b c                         | Créer une copie indirecte dans un split                 |
| b C                         | Cloner un buffer indirect dans une nouvelle fenêtre     |
| b d                         | Supprimer un bookmark                                   |
| b f                         | Fermer le buffer actuel                                 |
| b i                         | Ibuffer                                                 |
| b j                         | Buffer précédent                                        |
| b k                         | Buffer suivant                                          |
| b K                         | Fermer plusieurs buffers                                |
| b l                         | Lister les bookmarks                                    |
| b m                         | Définir un bookmark                                     |
...
| *Capture*                     |                                                         |
| m c                         | Menu Capture                                            |
| m c b                       | Capture – modèle b                                      |
| m c j                       | Capture – modèle j                                      |
| m c p                       | Capture – modèle p                                      |
| m c s                       | Capture – modèle s                                      |
| m c w                       | Capture – modèle w                                      |
|                             |                                                         |
| *Dates / Deadlines*           |                                                         |
| m d                         | Menu Dates & Deadlines                                  |
| m d d                       | Définir un deadline                                     |
| m d i                       | Insérer un timestamp inactif                            |
| m d n                       | Timestamp -1 jour                                       |
| m d p                       | Timestamp +1 jour                                       |
| m d r                       | Rapport de clocking                                     |
| m d s                       | Programmer (schedule)                                   |
| m d t                       | Insérer un timestamp                                    |
|                             |                                                         |
| *Ediff / Eshell / Eval / EWW* |                                                         |
| e                           | Menu Ediff / Eshell / Eval / EWW                        |
| e b                         | Eval buffer (évaluer le buffer)                         |
| e d                         | Eval defun (évaluer la fonction sous ou après le point) |
| e e                         | Eval expression (évaluer une expression elisp)          |
| e f                         | Ediff fichiers                                          |
| e F                         | Ediff trois fichiers                                    |
| e h                         | Historique Eshell                                       |
| e l                         | Eval last sexp (évaluer l’expression avant le point)    |
| e n                         | Nouveau buffer Eshell                                   |
| e r                         | Eval region (évaluer la région)                         |
...
| f d                         | Rechercher une chaîne dans les fichiers d’un répertoire |
| f e                         | Ouvrir le répertoire Emacs utilisateur dans Dired       |
| f g                         | Rechercher une chaîne dans le fichier courant           |
| f i                         | Ouvrir init.el                                          |
| f j                         | Aller à un fichier sous le répertoire courant           |
| f l                         | Localiser un fichier                                    |
| f r                         | Fichiers récents                                        |
...
| g c                         | Créer                                                   |
| g c b                       | Créer une branche et y passer                           |
| g c c                       | Créer un commit                                         |
| g c f                       | Créer un commit fixup                                   |
| g C                         | Cloner un dépôt                                         |
| g f                         | Trouver                                                 |
...
| g F                         | Git fetch                                               |
| g g                         | Magit status                                            |
| g i                         | Initialiser un dépôt git                                |
| g l                         | Journal du buffer git                                   |
| g r                         | Revert (annuler) un fichier git                         |
| g s                         | Stage (mettre en scène) un fichier git                  |
| g t                         | Git time machine                                        |
| g u                         | Unstage (désindexer) un fichier git                     |
|                             |                                                         |
| *Help*                        |                                                         |
| h                           | Menu Aide                                               |
| h a                         | Apropos (recherche d'aide par mots-clés)                |
| h b                         | Décrire les raccourcis (describe-bindings)              |
| h c                         | Décrire le caractère sous le curseur                    |
| h d                         | Documentation Emacs                                     |
| h d a                       | À propos d’Emacs                                        |
| h d d                       | Debugging Emacs                                         |
| h d f                       | FAQ Emacs                                               |
| h d m                       | Manuel Emacs                                            |
| h d n                       | Nouvelles d’Emacs                                       |
| h d o                       | Comment obtenir Emacs                                   |
| h d p                       | Problèmes connus d’Emacs                                |
| h d t                       | TODO officiel d’Emacs                                   |
| h d w                       | Licence / absence de garantie                           |
| h e                         | Messages de la zone d’écho                              |
| h f                         | Décrire une fonction                                    |
| h F                         | Décrire une face                                        |
| h g                         | Infos sur le projet GNU                                 |
| h i                         | Ouvrir Info                                             |
| h I                         | Décrire la méthode de saisie                            |
| h k                         | Décrire une touche                                      |
| h l                         | Voir les dernières commandes tapées (lossage)           |
| h L                         | Décrire l’environnement linguistique                    |
| h m                         | Décrire le mode courant                                 |
| h r                         | Recharger                                               |
| h r r                       | Recharger la configuration Emacs                        |
| h t                         | Charger un thème                                        |
| h v                         | Décrire une variable                                    |
| h w                         | Trouver le raccourci associé à une commande             |
| h x                         | Décrire une commande                                    |
|                             |                                                         |
| *Leader de base*              |                                                         |
| .                           | Ouvrir un fichier                                       |
...
| o F                         | Sélectionner un frame par son nom                       |
|                             |                                                         |
| *Org*                         |                                                         |
| m                           | Menu Org                                                |
| m a                         | Agenda Org                                              |
...
| m i                         | Basculer item (case à cocher / liste)                   |
| m l                         | Enregistrer un lien Org                                 |
| m t                         | Changer l’état TODO                                     |
| m T                         | Voir la liste des TODO                                  |
|                             |                                                         |
| *Tables*                      |                                                         |
| m b                         | Menu Tables                                             |
| m b -                       | Insérer une ligne horizontale                           |
| m b a                       | Aligner le tableau                                      |
| m b c                       | Insérer une colonne                                     |
| m b e                       | Modifier le champ courant                               |
| m b r                       | Insérer une ligne                                       |
...
| s o                         | Rechercher une chaîne dans un PDF                       |
| s t                         | Afficher la page TLDR d’une commande                    |
| s w                         | Ouvrir Woman (similaire à man sans dépendances)         |
|                             |                                                         |
| *Toggle*                      |                                                         |
| t                           | Menu Basculer                                           |
| t e                         | Activer/désactiver eshell                               |
| t f                         | Activer/désactiver Flycheck                             |
| t l                         | Activer/désactiver les numéros de ligne                 |
| t n                         | Activer/désactiver Neotree                              |
| t o                         | Activer/désactiver Org mode                             |
| t r                         | Activer/désactiver Rainbow mode                         |
| t t                         | Activer/désactiver visual-line-mode                     |
| t v                         | Activer/désactiver vterm                                |
|                             |                                                         |
| *Windows / Words*             |                                                         |
| w                           | Menu Fenêtres & Mots                                    |
| w -                         | Split horizontal                                        |
| w H                         | Déplacer la fenêtre à gauche                            |
| w J                         | Déplacer la fenêtre en bas                              |
| w K                         | Déplacer la fenêtre en haut                             |
| w L                         | Déplacer la fenêtre à droite                            |
| w d                         | Mettre le mot en minuscules                             |
| w f                         | Fermer la fenêtre                                       |
| w h                         | Aller à la fenêtre de gauche                            |
| w j                         | Aller à la fenêtre du bas                               |
| w k                         | Aller à la fenêtre du haut                              |
| w l                         | Aller à la fenêtre de droite                            |
| w n                         | Nouvelle fenêtre                                        |
| w u                         | Mettre le mot en majuscules                             |
| w v                         | Split vertical                                          |
| w w                         | Aller à la fenêtre suivante                             |
...
*** Kloc
En faisant M-x select-kbloc, il selectionne le bloc commençant par #+BEGIN_SRC jusqu'à 
...
* Clique
** Activer la synchronisation entre emacs, evil et le presse-papiers système
*** Mode terminal
#+begin_src emacs-lisp
...
* Desactiver l'écran d'accueil d'emacs
#+begin_src emacs-lisp
...
| œn   | #+name:                             |
| œ    | <                                   |
| œfh  | fusion horizontale de deux tableaux |
|      |                                     |
#+BEGIN_SRC emacs-lisp
...
| n ou k / p ou j | Déplacer le curseur vers le haut / bas |
| TAB             | Déplier / refermer un dossier          |
| RET             | Ouvrir le fichier                      |
| q               | Fermer NeoTree                         |
| r               | Rafraîchir l’arbre                     |
| d               | Supprimer fichier                      |
| c               | Créer fichier/dossier                  |
|-----------------+----------------------------------------|
...
#+RESULTS:
: ((fullscreen . maximized) (vertical-scroll-bars))
...
| Élément            | Effet                                     |
|--------------------+-------------------------------------------|
| #+PROPERTY:        | Définit une propriété globale du document |
| header-args        | S’applique à TOUS les blocs `#+begin_src`  |
| header-args:python | S’applique uniquement aux blocs Python    |
| :session my_session | Les blocs partagent la même session       |
|                    | (variables persistantes)                  |
| :exports both      | Code + résultats inclus à l’export        |
| :exports code      | Seulement le code à l’export              |
| :exports results   | Seulement les résultats à l’export        |
| :exports none      | Rien n’est exporté                        |

+ #+PROPERTY: header-args :session my_session :exports both        # # accepte tous les langages


| :results                          | Ce que Org récupère                   | Exemple Python                | Résultat Org                    |
|-----------------------------------+---------------------------------------+-------------------------------+---------------------------------|
| output                            | Texte affiché (stdout)                | print("Hi")                   | Hi                              |
| value                             | Valeur retournée                      | 2 + 2                         | 4                               |
| file                              | Nom de fichier généré                 | ["data.csv"]                  | [[file:data.csv]]                   |
| table                             | Tableau Org (liste de listes)         | [[1,2],[3,4]]                 |                                 |
| table replace                     | Tableau Org, remplace ancien résultat | [[1,2],[3,4]] → [[5,6],[7,8]] |                                 |
| graphics file :file /tmp/plot.png | Image générée dans un fichier         | matplotlib plot               | [[file:/tmp/plot.png]]              |
| raw                               | Insère le résultat tel quel           | "<b>Hi</b>"                   | <b>Hi</b>                       |
| silent                            | Rien n’est inséré                     | print("Hi")                   | (rien)                          |
| drawer                            | Résultat dans un drawer               | 2 + 2                         | :RESULTS:\n4\n:END:             |
| replace                           | Remplace l’ancien résultat            | print("Hi")                   | Hi (remplace l’ancien résultat) |
| append                            | Ajoute après l’ancien résultat        | print("Hi")                   | Ancien résultat\nHi             |
| prepend                           | Ajoute avant l’ancien résultat        | print("Hi")                   | Hi\nAncien résultat             |
...
| Ligne                   | Portée / effet                                    |
|-------------------------+---------------------------------------------------|
| #+HEADER:               | locale → seulement le bloc suivant                |
| #+PROPERTY: header-args | globale → tous les blocs du langage ou du fichier |
...
| Option                     | Rôle / Explication                                    | Exemple Python                                       | Résultat Org                        |
|----------------------------+------------------------------------------------------+----------------------------------------------------+------------------------------------|
| :var <nom>=<val>           | Passe une variable dans le bloc source              | :var dirs=["src","tests"]                           | dirs disponible dans le bloc       |
| :results <type>            | Définit le type de résultat (value, output, table, file, graphics) | :results output                                    | texte affiché par print()          |
| :session <nom>             | Spécifie la session partagée (persistante)          | :session my_session                                 | variables persistantes entre blocs |
| :exports <mode>            | Ce qui sera exporté : code, results, both          | :exports both                                      | export code + résultat             |
| :file <chemin>             | Spécifie un fichier de sortie                       | :file /tmp/plot.png                                | lien vers le fichier [[file:/tmp/plot.png]] |
| :results table             | Force un tableau Org                                 | [[1,2],[3,4]]                                      | |1|2|\n|3|4|                        |
| :results table replace     | Tableau Org, remplace l’ancien résultat             | [[5,6],[7,8]]                                      | |5|6|\n|7|8|                        |
| :results graphics file :file <chemin> | Génère un graphique / image dans un fichier | matplotlib plot :file /tmp/plot.png               | [[file:/tmp/plot.png]]             |
| :results raw               | Insère le résultat tel quel                          | "<b>Hi</b>"                                        | <b>Hi</b>                          |
| :results silent            | N’insère rien                                        | print("Hi")                                        | (rien)                             |
| :results drawer            | Met le résultat dans un drawer                       | 2 + 2                                              | :RESULTS:\n4\n:END:                |
| :results append            | Ajoute après l’ancien résultat                        | print("Hi")                                        | Ancien résultat\nHi                 |
| :results prepend           | Ajoute avant l’ancien résultat                        | print("Hi")                                        | Hi\nAncien résultat                 |
| :results list              | Force une liste                                     | [1,2,3]                                           | - 1\n- 2\n- 3                       |
| :results scalar            | Valeur simple                                       | 42                                                | 42                                  |

** Tampon python
+ C-c'   ouverture et fermeture
** 1. Chargement des librairies

#+BEGIN_SRC python :results output
import matplotlib
matplotlib.use('Agg')  # Backend non interactif (nécessaire pour l'export PNG)
...
# Renvoie une liste de carrés de 0 à 9
...
| e (ge) | aller à la fin de chaque mot (reculer) |
|--------+----------------------------------------|
**** sur les lignes
| normal | emacs     | Description                              |
|--------+-----------+------------------------------------------|
| 0  (_) | C-a (M-m) | aller au debut de la ligne (sans espace) |
| $ (g_) | C-e       | aller à la fin de la ligne (sans espace) |
|        | M-a (M-e) | aller au debut (fin)  de la phrase       |
|--------+-----------+------------------------------------------|

Une ~phrase~ en emacs commence par un caractère autre que #, espace et
se termine soit par .,! ou ? suivi d'un espace ou retour à la ligne.
...
| normal | Description                          |
|--------+--------------------------------------|
| gg (G) | aller au debut du fichier (à la fin) |
|--------+--------------------------------------|
**** aller jusqu'au
***** caractère x exlus
+ tx
...
|------------+--------------------------------|
| iw         | intérieur du mot               |
| i<symbole> | intérieur du symbole           |
| a<symbole> | symbole + intérieur du symbole |
|------------+--------------------------------|
**** sur l'ecran
| normal     | Description                                                    |
|------------+----------------------------------------------------------------|
...
| zz         | mettre la ligne courante au mileu de l'écran                   |
| zt         | mettre la ligne courante au top  de l'écran                    |
| zb         | mettre la ligne courante en bas  de l'écran                    |
|------------+----------------------------------------------------------------|
**** sur les headings
| commande        | action                                             |
|-----------------+----------------------------------------------------|
...
| M-entrer        | ajouter un heading de même niveau que le precedent |
|-----------------+----------------------------------------------------|
...
|-----------+-----------------------------------|
| i ou a    | inserer avant ou après le curseur |
...
| v      | selectionner caractère par caractère          |
| C-v    | selectionner par bloc                         |
|--------+-----------------------------------------------|
**** opération sur les selection par bloc ou caractère 
***** pour copier ou supprimer
| Visual | Description                               |
|--------+-------------------------------------------|
| tx     | aller jusqu'avant le caractère x          |
| fx     | aller jusqu'au caractère x                |
| $      | aller jusqu'à la fin de ligne             |
| g_     | aller juqu'à la fin de ligne sans espace  |
| 0      | aller jusqu'à debut de ligne             |
| _      | aller jusqu'en debut de ligne sans espace |
|--------+-------------------------------------------|
***** faire une action après le bloc
| Visual | Description      |
|--------+------------------|
| A      | après le bloc    |
| I      | avant le bloc    |
...
| Action                           | Normal | Emacs |
|----------------------------------+--------+-------|
| coller avant ou après le curseur | P ou p | C-y   |
|----------------------------------+--------+-------|
*** Supprimer = couper
 | Normal  | Action                                       |
 |---------+----------------------------------------------|
 ...
 | S-d     | supprimer à partir du curseur jusqu'à la fin |
 |---------+----------------------------------------------|
...
|--------------+-------------+-----------------|
*** Mode commande
| Commande | Usage                          | Fonction                                                       | Exemple           |
|----------+--------------------------------+----------------------------------------------------------------+-------------------|
| :r       | :r <fichier>                   | Insère le contenu d’un fichier après la ligne courante         | :r monfichier.txt |
| :r !     | :r !<commande>                 | Insère le résultat d’une commande shell                        | :r !date          |
| :put     | :put [registre]                | Colle le contenu d’un registre après la ligne courante         | :put a            |
| :new     | :new                           | Ouvre une nouvelle fenêtre (split) vide                        | :new              |
| :q       | :q                             | Quitte le buffer actuel                                        | :q                |
| :q!      | :q!                            | Quitte sans sauvegarder                                        | :q!               |
| :w       | :w [fichier]                   | Sauvegarde le buffer dans un fichier                           | :w                |
| :!       | :!<commande>                   | Exécute une commande shell externe                             | :!ls              |
| :%s      | :%s/<ancien>/<nouveau>/[flags] | Remplace du texte dans le buffer entier ou une plage de lignes | :%s/chien/chat/g  |
**** flags
| Flag | Fonction                                                  | Exemple                                                                 |
|------+-----------------------------------------------------------+-------------------------------------------------------------------------|
| g    | global : remplace toutes les occurrences sur chaque ligne | :%s/chat/chien/g remplace tous les “chat” par “chien” dans chaque ligne |
| c    | confirm : demande confirmation pour chaque remplacement   | :%s/chat/chien/gc te demande “oui/non” pour chaque “chat” trouvé        |
| i    | ignore case : ignore la casse                             | :%s/Chat/chien/gi remplace “Chat”, “chat”, “CHAT”, etc.                 |
| I    | respect case : sensible à la casse (par défaut)           | :%s/Chat/chien/I remplace uniquement “Chat” exactement                  |
| n    | ne rien remplacer, juste compter                          | :%s/chat/chien/n affiche le nombre de remplacements possibles           |
| &    | réutiliser les flags de la dernière substitution          | :%s/chat/chien/& applique les mêmes flags que la dernière commande :s   |
...
| commande          | action                        |
|-------------------+-------------------------------|
| M-S-flèche gauche | supprimer la colonne courante |
| M-S-flèche haute  | supprimer la ligne courante   |
|-------------------+-------------------------------|
***** ajouter une colonne ou une ligne
| commande          | action                       |
|-------------------+------------------------------|
| M-S-flèche droite | ajouter une colonne courante |
| M-S-flèche bas    | ajouter une  ligne courante  |
|-------------------+------------------------------|
***** tableau vide
C-c | + nbre__colonne x nbre__ligne  
***** données brutes en tableau
...
**** Permuter les lignes ou colonnes / les éléments du tableau
| commande         | action                          |
|------------------+---------------------------------|
| M- + les flèches | permuter les lignes ou colonnes |
| S- + les flèches | permuter les elts du tableau    |
|------------------+---------------------------------|
...
| Type d’opération   | Syntaxe              | Explication                                                          |
|--------------------+----------------------+----------------------------------------------------------------------|
| Colonne (vertical) | $3 = $1 + $2         | La colonne 3 = col1 + col2                                           |
| Ligne (horizontal) | @2 = @1 * 2          | La ligne 2 recevra touts les valeurs de la ligne 1 multipliéés par 2 |
| Rectangle          | @2$2..@4$3 = $1 + 10 | Cette formule est appliquée à chaque cellule du rectangle            |
| Rectangle          | @2$2..@4$3 = expr    | Applique expr au bloc ligne 2→4, colonne 2→3                         |
| Cumul colonne      | $2 += 1              | Ajoute 1 à chaque cellule de la colonne 2                            |
...
|---------------------+------------------+------------------------------------+---------------------------|
| Somme verticale     | vsum(range)      | Somme des valeurs d’une colonne    | @>$2 = vsum(@2..@-1)      |
| Moyenne verticale   | vmean(range)     | Moyenne des valeurs d’une colonne  | @>$2 = vmean(@2..@-1)     |
| Valeur min          | vmin(range)      | Valeur minimale d’une colonne      | @>$2 = vmin(@2..@-1)      |
| Valeur max          | vmax(range)      | Valeur maximale d’une colonne      | @>$2 = vmax(@2..@-1)      |
| Compte              | vcount(range)    | Nombre de valeurs numériques       | @>$2 = vcount(@2..@-1)    |
| Produit             | vprod(range)     | Produit des valeurs d’une colonne  | @>$2 = vprod(@2..@-1)     |
| Somme des carrés    | vsumsq(range)    | Somme des carrés des valeurs       | @>$2 = vsumsq(@2..@-1)    |
| Écart-type          | vstd(range)      | Écart-type des valeurs             | @>$2 = vstd(@2..@-1)      |
| Variance            | vvariance(range) | Variance des valeurs               | @>$2 = vvariance(@2..@-1) |
| Médiane verticale   | vmedian(range)   | Médiane d’une colonne              | @>$2 = vmedian(@2..@-1)   |
| Somme horizontale   | hsum(range)      | Somme des valeurs d’une ligne      | @2$4 = hsum($1..$3)       |
| Moyenne horizontale | hmean(range)     | Moyenne des valeurs d’une ligne    | @2$4 = hmean($1..$3)      |
| Valeur min ligne    | hmin(range)      | Minimum des valeurs d’une ligne    | @2$4 = hmin($1..$3)       |
| Valeur max ligne    | hmax(range)      | Maximum des valeurs d’une ligne    | @2$4 = hmax($1..$3)       |
| Compte ligne        | hcount(range)    | Nombre de valeurs numériques ligne | @2$4 = hcount($1..$3)     |
|---------------------+------------------+------------------------------------+---------------------------|
**** Sur les blocs
***** syntaxe
...
***** les options
Note: X peut être le nom d'une table ou un extrait de la table

| option                 | description                                                                             |
|------------------------+-----------------------------------------------------------------------------------------|
| :var tab=X             | recupère l'objet X et stocke   dans tab                                                 |
| :colnames yes          | n'applique pas l'action sur la 1ère ligne du tableau                                    |
| :colnames no           | applique l'action sur la 1ère ligne du tableau aussi                                    |
| :results output        | affiche la sortie (à utiliser avec print ou printf)                                     |
| table                  | convertir le output en tableau org                                                      |
| :results graphics file | le résultat est un fichier graphique                                                    |
| :file chemin/image.png | définit l’emplacement et le nom du fichier généré                                       |
| :exports code          | n’exporte que le code du bloc                                                           |
| :exports results       | n’exporte que les résultats du bloc                                                     |
| :exports both          | exporte le code et les résultats.                                                       |
| :results table         | Formate le résultat comme tableau Org                                                   |
| :results value         | Récupère la valeur renvoyée par l’expression, utilisable dans d’autres blocs ou calculs |
|------------------------+-----------------------------------------------------------------------------------------|
...
**** Nommer un tableau
#+name: sert à nommer un tableau ou la sortie du bloc
...
|-------------+--------------------------------------+-----------------------------------------------------------------|
| SPC SPC     | Counsel M-x                          | Exécute une commande Emacs via interface de recherche           |
| SPC .       | Find file                            | Ouvre un fichier depuis le système de fichiers                  |
| SPC =       | Perspective                          | Ouvre le menu de gestion des espaces de travail                 |
| SPC TAB TAB | Comment lines                        | Commente ou décommente la/les ligne(s) courante(s)              |
| SPC u       | Universal argument                   | Fournit un argument numérique universel à la prochaine commande |
| SPC a       | A.I.                                 | Ouvre le sous-menu des fonctions d’intelligence artificielle    |
| SPC a a     | Ask ellama about region              | Demande à Ellama des informations sur la région sélectionnée    |
| SPC a e     | Ellama enhance                       | Ouvre le sous-menu d’amélioration de texte via Ellama           |
| SPC a e g   | Ellama improve grammar               | Corrige la grammaire du texte sélectionné                       |
| SPC a e w   | Ellama improve wording               | Améliore le style ou le choix des mots du texte sélectionné     |
| SPC a i     | Ellama chat                          | Lance une session de discussion avec Ellama                     |
| SPC a p     | Ellama provider select               | Change le fournisseur de modèle IA (Ellama)                     |
| SPC a s     | Ellama summarize region              | Résume le texte sélectionné                                     |
| SPC a t     | Ellama translate region              | Traduit le texte sélectionné                                    |
...
| SPC b C     | Clone indirect buffer (other window) | Duplication dans autre fenêtre                                  |
...
| SPC b m     | Bookmark set                         | Créer un signet                                                 |
| SPC b k     | Next buffer                          | Naviguer vers le suivant                                        |
| SPC b j     | Previous buffer                      | Naviguer vers le précédent                                      |
...
| SPC d f     | Finish wdired edit                   | Finir édition dans Dired                                        |
| SPC d j     | Dired jump                           | Aller à dossier du buffer                                       |
| SPC d n     | Neotree dir                          | Ouvrir dans Neotree                                             |
| SPC d p     | Peep Dired                           | Aperçu des fichiers                                             |
| SPC d w     | Writable Dired                       | Mode édition Dired                                              |
| SPC e       | Ediff / Eshell / Eval / EWW          | Évaluation & shell                                              |
| SPC e b     | Eval buffer                          | Évaluer tout le buffer                                          |
| SPC e d     | Eval defun                           | Évaluer la fonction actuelle                                    |
| SPC e e     | Eval expression                      | Évaluer une expression                                          |
...
| SPC e l     | Eval last sexp                       | Évaluer dernière expression                                     |
| SPC e n     | Eshell new                           | Nouveau terminal Eshell                                         |
| SPC e r     | Eval region                          | Évaluer une région                                              |
...
| SPC f e     | Open ~/.emacs.d in Dired             | Ouvrir répertoire Emacs                                         |
...
| SPC f j     | File jump                            | Aller à un fichier                                              |
| SPC f l     | Locate file                          | Trouver un fichier                                              |
| SPC f r     | Counsel recentf                      | Fichiers récents                                                |
| SPC f u     | Sudo find file                       | Ouvrir fichier en root                                          |
| SPC f U     | Sudo edit file                       | Éditer fichier en root                                          |
...
| SPC g c     | Create submenu                       | Création Git                                                    |
| SPC g c b   | Branch and checkout                  | Créer et basculer branche                                       |
| SPC g c c   | Commit create                        | Créer un commit                                                 |
| SPC g c f   | Commit fixup                         | Fixup commit                                                    |
| SPC g C     | Magit clone                          | Cloner un dépôt                                                 |
...
| SPC g F     | Git fetch                            | Récupérer modifications                                         |
| SPC g g     | Magit status                         | État du dépôt                                                   |
| SPC g i     | Magit init                           | Initialiser dépôt Git                                           |
| SPC g l     | Log buffer file                      | Historique Git du fichier                                       |
| SPC g r     | VC revert                            | Revenir à version précédente                                    |
...
| SPC h c     | Describe char                        | Infos sur caractère                                             |
| SPC h d     | Documentation                        | Documentation Emacs                                             |
| SPC h d a   | About Emacs                          | À propos d’Emacs                                                |
| SPC h d d   | View debugging                       | Débogage                                                        |
| SPC h d f   | View FAQ                             | Aide                                                            |
| SPC h d m   | Manual                               | Manuel officiel                                                 |
| SPC h d n   | News                                 | Nouveautés Emacs                                                |
| SPC h d o   | Distribution info                    | Comment obtenir Emacs                                           |
| SPC h d p   | Problems                             | Problèmes connus                                                |
| SPC h d t   | Todo list                            | Liste de tâches internes                                        |
| SPC h d w   | No warranty                          | Clause de non-garantie                                          |
| SPC h e     | Echo messages                        | Voir messages d’écho                                            |
| SPC h f     | Describe function                    | Décrire une fonction                                            |
| SPC h F     | Describe face                        | Décrire un thème/fonte                                          |
| SPC h g     | Describe GNU project                 | Infos sur GNU                                                   |
| SPC h i     | Info browser                         | Navigateur d’aide                                               |
| SPC h I     | Describe input method                | Méthode d’entrée                                                |
| SPC h k     | Describe key                         | Voir quelle commande est liée                                   |
...
| SPC o d     | Dashboard                            | Écran d’accueil                                                 |
| SPC o e     | Elfeed RSS                           | Lecteur de flux RSS                                             |
| SPC o f     | Open buffer in frame                 | Ouvrir dans une autre fenêtre                                   |
| SPC o F     | Select frame by name                 | Sélection de fenêtre                                            |
| SPC p       | Projectile command map               | Projets                                                         |
| SPC r p     | Radio play                           | Démarrer la radio                                               |
| SPC r s     | Radio stop                           | Arrêter la radio                                                |
...
| SPC s t     | TLDR docs                            | Résumé commandes shell                                          |
| SPC s w     | Woman                                | Documentation interne                                           |
| SPC t e     | Eshell toggle                        | Terminal intégré                                                |
| SPC t f     | Flycheck mode                        | Vérification syntaxique                                         |
| SPC t l     | Display line numbers                 | Numéros de ligne                                                |
...
| SPC w f     | Evil window delete                   | Fermer fenêtre                                                  |
| SPC w n     | Evil window new                      | Nouvelle fenêtre                                                |
| SPC w -     | Horizontal split                     | Séparation horizontale                                          |
| SPC w v     | Vertical split                       | Séparation verticale                                            |
| SPC w h     | Window left                          | Aller à gauche                                                  |
| SPC w j     | Window down                          | Aller en bas                                                    |
| SPC w k     | Window up                            | Aller en haut                                                   |
| SPC w l     | Window right                         | Aller à droite                                                  |
| SPC w w     | Window next                          | Fenêtre suivante                                                |
| SPC w H     | Buffer move left                     | Déplacer à gauche                                               |
| SPC w J     | Buffer move down                     | Déplacer en bas                                                 |
| SPC w K     | Buffer move up                       | Déplacer en haut                                                |
| SPC w L     | Buffer move right                    | Déplacer à droite                                               |
...
+ unmodified : les fichiers non modifiés par rapport au dernier commit.
+ modified : les fichiers modifiés par rapport au dernier commit.
+ stash : lieu où l'on peut stocker les fichiers modifiés.
***** Dossier non suivi
+ untracked : les fichiers n'ayant jamais subi git add.
**** Stage
Lieu où les fichiers sont capturés/copiés.
***** Envoyer les fichiers ou dossier dans le stage
****** Envoyer un dossier ou fichier spécifique
+ git add nom__fichier ou git add nom__dossier/
+ git add -f nom__dossier/                     # Forcer git à envoyer le dossier sans rien enlever
+ git add -f nom__dossier/nom__fichier          # Forcer git à ajouter nom__fichier dans nom__dossier
...
**** Repository
Lieu où les captures sont enregistrées.
***** Enregistrer un commit
+ git commit -m"commentaire à faire"
***** Annuler un commit
+ git reset --soft HEAD~1   # uniquement le commit
+ git reset --mixed HEAD~1  # uniquement le commit et le staged
**** Stash
Lorsqu'un fichier modifié est envoyé au stash,
notre tracked file pointe sur le dernier commit.
***** Envoyer un fichier dans le stash
...
**** Repository distant
Lieu où les fichiers sont enregistrés sur le net.
...
+ aller sur la branche à mettre à jour car HEAD suit le sens du 1er parent.
+ git merge
***** Avec rebase : transfère tous les commits de la branche courant vers l'autre
+ aller sur la branche qui enverrais les commits
+ git rebase
**** Envoyer les fichiers vers le depot distant à travers une branche tierce
+ git push origin nom__branche__tierce

***  Etat des fichiers
**** git status
+ montre l'état des fichiers tracked et untracked
+ montre les conflis s'il existe
**** git diff
compare le fichier modifié au dernier staged
**** git --staged
compare le dernier staged et le dernier commit
**** git diff HEAD
compare le fichier modifié au dernier commit
*** Les conflits
**** Les zones dans git
Supposons que nous avons un fichier ancetre dont les lignes sont
contituées de A,B,C et D et lig une ligne insérée.
+ si lig est avant A, alors la zone est "zone avant A"
+ si lig est entre A et B, alors la zone est "zone entre A et B"
+ si lig est après D, alors la zone est "zone après D"
+ si B est modifiées, git considere que la ligne B a été supprimée donc la "zone B" et
  la nouvelle ligne est dans la zone "zone entre A et C"
**** Règle de conflits
Il y'a conflit lorsqu'une même zone est modifiée.
...
+ git push --force
*** Customiser git
**** De manière globale (le bash)
Les couleurs sont à mettre en hexadecimales
# Untracked files → noir sur jaune
+ git config --global color.status.untracked "black #ffff00"
# Modified → jaune sur noir
+ git config --global color.status.changed "#ff66cc"
# Added (staged) → vert vif
+ git config --global color.status.added "#55ff00"
# En conflit
+ git config --global color.status.unmerged "magenta italic"
# Deleted → rouge vif
+ git config --global color.status.deleted "red bold underline"
# Branch name (in status output)
+ git config --global color.branch.current "yellow bold"
**** De manière locale (.git.config)
+ st = status -s
+ graph = log --oneline --graph --all
+ acp = "!f() { git add . && git commit -m \"$*\" && git push; }; f"
** Cheat sheet sur la capture
**** Pour capture
| Code     | Nom       | Fichier cible | Description                          | Exemple de note générée                                   |
|----------+-----------+---------------+--------------------------------------+-----------------------------------------------------------|
| SPC c  b | Brouillon | brouillon.org | Note rapide ou brouillon             | * TODO Relire le chapitre 3\n 2025-12-08 Tue 14:30        |
| SPC c  p | Personal  | personal.org  | Notes personnelles                   | * TODO Acheter du lait\n 2025-12-08 Tue 14:31             |
| SPC c  w | Work      | work.org      | Notes liées au travail               | * TODO Envoyer le rapport\n 2025-12-08 Tue 14:32          |
| SPC c  s | School    | school.org    | Notes scolaires ou devoirs           | * TODO Préparer le devoir de maths\n 2025-12-08 Tue 14:33 |
| SPC c  j | Journal   | journal.org   | Entrée de journal organisée par date | * Réflexion du jour\nEntered on 2025-12-08 Tue 14:34      |
|----------+-----------+---------------+--------------------------------------+-----------------------------------------------------------|
**** Pour timestamp
| Commande / Template                      | Élément                  | Description / Usage                                                                           |
|------------------------------------------+--------------------------+-----------------------------------------------------------------------------------------------|
| C-c .                                    | Timestamp actif          | Insère un timestamp actif <YYYY-MM-DD> ou <YYYY-MM-DD HH:MM>. Apparaît dans l’agenda.         |
| C-c !                                    | Timestamp inactif        | Insère un timestamp inactif [YYYY-MM-DD]. Référence historique, n’apparaît pas dans l’agenda. |
| S-<left> / S-<right> / S-<up> / S-<down> | Déplacer timestamp actif | Déplace la date du timestamp actif d’un jour ou d’un mois.                                    |
| C-c C-d                                  | Deadline                 | Définit la date limite de la tâche. Apparaît dans l’agenda avec rappel.                       |
| C-c C-s                                  | Schedule                 | Définit la date de début de la tâche. Apparaît dans l’agenda à partir de cette date.          |
| C-c C-x C-i                              | Intervalle               | Insère un intervalle de dates <2025-12-08 10:00-12:00>.                                       |
|------------------------------------------+--------------------------+-----------------------------------------------------------------------------------------------|

| Type de TODO                       | Exemple Org-mode                                 | Comportement dans l’agenda                                                           |
|------------------------------------+--------------------------------------------------+---------------------------------------------------------------------------------------|
| TODO simple (timestamp actif)      | * TODO Préparer le rapport <2025-12-08 Mon>      | La tâche apparaît uniquement à la date du timestamp.                                  |
| TODO avec SCHEDULED                | * TODO Préparer le rapport                       | La tâche apparaît à partir de la date scheduled et reste visible jusqu’à ce qu’elle   |
|                                    |   SCHEDULED: <2025-12-05 Fri>                    | soit faite.                                                                          |
| TODO avec DEADLINE                 | * TODO Préparer le rapport                       | La tâche doit être terminée au plus tard à cette date ; alerte si en retard.          |
|                                    |   DEADLINE: <2025-12-08 Mon>                     |                                                                                       |
| TODO complet (Scheduled + Deadline)| * TODO Préparer le rapport                       | La tâche apparaît à partir de la date scheduled et doit être terminée avant la        |
|                                    |   SCHEDULED: <2025-12-05 Fri>                    | deadline.                                                                             |
|                                    |   DEADLINE: <2025-12-08 Mon>                     |                                                                                       |
|------------------------------------+--------------------------------------------------+---------------------------------------------------------------------------------------|
C-c C-t  permet de basculer entre DONE et TODO.

| Type de répétition  | Syntaxe | Exemple initial             | Comment ça se comporte si la tâche n’est pas faite                                                         |
|---------------------+---------+-----------------------------+------------------------------------------------------------------------------------------------------------|
| Standard            |     +1d | <2025-12-08 Mon 08:00 +1d>  | Les jours manqués ne sont pas rattrapés. La prochaine occurrence reste fixée depuis la date initiale.      |
| Basé sur complétion |    ++1d | <2025-12-08 Mon 08:00 ++1d> | La répétition se calcule depuis la date où la tâche est marquée DONE. Les jours manqués sont ignorés.      |
| Rattrapage          |    .+1d | <2025-12-08 Mon 08:00 .+1d> | Toutes les occurrences manquées apparaissent dans l’agenda. La prochaine occurrence part de la complétion. |
|---------------------+---------+-----------------------------+------------------------------------------------------------------------------------------------------------|
** Cheat sheet: buffer fenêtre bookmarks
*** Fenêtres
| Commande | Action                             | Description                                          |
|----------+------------------------------------+------------------------------------------------------|
| C-x 1    | Supprimer les autres fenêtres      | Garde la fenêtre courante et ferme toutes les autres |
| C-x 0    | Supprimer cette fenêtre            | Ferme la fenêtre courante                            |
| C-x 2    | Diviser la fenêtre horizontalement | Coupe la fenêtre en deux (haut / bas)                |
| C-x 3    | Diviser la fenêtre verticalement   | Coupe la fenêtre en deux (gauche / droite)           |
| C-c n    | Aller à la fenêtre numéro n        | Active la fenêtre numérotée n (ex: C-c 2)            |
*** Buffers ou tampons
| Commande | Action                        | Description                                                 |
|----------+-------------------------------+-------------------------------------------------------------|
| C-x b    | Changer de tampon             | Bascule vers un autre tampon existant ou en crée un nouveau |
| C-x C-b  | Lister les tampons            | Affiche la liste de tous les tampons ouverts (*Buffer List*)  |
| C-x k    | Supprimer un tampon           | Ferme le tampon courant ou un tampon spécifié               |
| C-x C-s  | Sauvegarder                   | Enregistre le tampon courant dans son fichier               |
| C-x C-w  | Sauvegarder sous un autre nom | Écrit le tampon courant dans un nouveau fichier             |
*** Bookmarks
| Commande            | Action                | Description                                        |
|---------------------+-----------------------+----------------------------------------------------|
| C-c m               | Créer un bookmark     | Ajoute un signet à la position courante            |
| C-c g               | Aller à un bookmark   | Saute vers un signet existant                      |
| C-c b               | Lister les bookmarks  | Affiche la liste des signets                       |
| C-c d               | Supprimer un bookmark | Supprime un signet                                 |
| C-c s               | mode normal           | Sauvegarder les bookmarks dans le fichier bookmark |
+ Les bookmarks sont sauvegardés entre les sessions
**** Personnalisation
#+BEGIN_SRC emacs-lisp
  ;; Créer un bookmark
...
| Commande | Action               | Description                          |
|----------+----------------------+--------------------------------------|
| m{a-z}   | Créer un bookmark    | Marque la position courante (ex: ma) |
| '{a-z}   | Aller à un bookmark  | Va à la ligne du marqueur            |
| `{a-z}   | Aller exact          | Va à la ligne et colonne du marqueur |
| :marks   | Lister les bookmarks | Affiche tous les marqueurs           |
+ Les marques sont perdues à la fermeture (par défaut)                                          
** Cheat sheet mu4e
| Touche / Commande | Action                               | Exemple concret                | Résultat                     |
|-------------------+--------------------------------------+--------------------------------+------------------------------|
| M-x mu4e          | Lancer mu4e                          | M-x mu4e                       | Ouvre l’interface mail       |
| shift c           | Ecrire un mail                       |                                |                              |
| C-c C-a           | joindre un fichier                   |                                |                              |
| C-c C-k           | Sortir/annuler de la redaction(mail) |                                |                              |
| C-c C-c           | Envoyer un mail                      | Écrire → C-c C-c               | Envoi via SMTP               |
| RET               | Lire un mail ou afficher le pj       | RET sur un message             | Ouvre le mail                |
| Ji                | Aller dans l’INBOX                   | Ji → /Gmail/INBOX              | Affiche les mails reçus      |
| s                 | Rechercher un mail                   | s from:google subject:security | Liste des mails Google       |
| bu                | Voir mails non lus                   | bu                             | Affiche tous les non lus     |
| n                 | Aller au mail suivant                | n                              | Message suivant              |
| p                 | Aller au mail précédent              | p                              | Message précédent            |
| r                 | Répondre                             | r                              | Répond à l’expéditeur        |
| R                 | Répondre à tous                      | R                              | Répond à tous                |
| C                 | Nouveau mail                         | C                              | Ouvre le buffer de rédaction |
| !                 | Marquer comme lu                     | !                              | Marque le mail comme lu      |
| ?                 | Marquer non lu                       | ?                              | Marque non lu                |
| d                 | Supprimer (Gmail Trash)              | d                              | Déplace vers Corbeille       |
| a                 | Archiver                             | a                              | Archive le mail              |
| f                 | Ajouter un drapeau                   | f                              | Ajoute un flag               |
| A s               | Télécharger pièce jointe             | A s                            | Sauve la pièce jointe        |
| A                 | Voir pièces jointes                  | A                              | Liste des PJ                 |
| J                 | Changer de dossier                   | J → sélectionner un dossier    | Navigation rapide            |
| u                 | Mettre à jour les mails              | u                              | Sync + index                 |
| q                 | Quitter mu4e                         | q                              | Quitte mu4e                  |
| C-c C-d           | Enregistrer dans le brouillon        |                                |                              |

* Verification des commandes
| Commande | Action        | Description                                               |
|----------+---------------+-----------------------------------------------------------|
| C-h k    | describe-key  | Affiche la commande associée à une touche spécifique      |
| C-h m    | describe-mode | Affiche les raccourcis et fonctionnalités du mode courant |
